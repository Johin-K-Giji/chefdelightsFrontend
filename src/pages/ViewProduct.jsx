...
const ViewProducts = () => {
  ...
  const fetchProducts = () => {
    axios.get('https://chefdelightsfoods.com/api/products/')
      .then(response => {
  ...
  const handleEditSubmit = () => {
    ...
    axios
      .put(`https://chefdelightsfoods.com/api/products/edit/${editProduct._id}`, formData)
      .then((response) => {
  ...
  const handleDeleteClick = (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      axios.delete(`https://chefdelightsfoods.com/api/products/delete/${id}`)
        .then(() => fetchProducts())
        .catch(error => {
            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product, index) => (
                <tr key={product._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <img
                      src={`https://chefdelightsfoods.com/static/products/${product.coverImage}`}
                      alt="Cover"
                  ...
                  <td className="p-4">
                    <div className="flex">
                      {product.subImages?.map((img, idx) => (
                        <img
                          key={idx}
                          src={`https://chefdelightsfoods.com/static/products/${img}`}
                          alt="Sub"
              ...
              <label className="block mb-2">
                ...
                {existingCoverImage && (
                  <div className="flex items-center mt-1">
                    <img
                      src={`https://chefdelightsfoods.com/static/products/${existingCoverImage}`}
                      alt="Current Cover"
                      className="w-12 h-12 object-cover rounded mr-2"
                    />
                    <button
